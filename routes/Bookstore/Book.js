...
const BookApi = require("../../models/BookStore/Book/book")

router.get('/', async  (req, res)=> {
    try {
        const data = await BookApi.find({}) 
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const product = await BookApi.findById(req.params.id);
...
router.post('/', async (req, res) => {
    try {
        ...
        let data = (await BookApi.create(req.body))
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const data = await BookApi.updateOne(req.params,req.body)
...
router.delete('/:id', async (req, res) => {
    try {
        const data = await BookApi.findByIdAndDelete({_id: req.params.id});
